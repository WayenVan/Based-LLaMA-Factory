...
class TranslateMetric:
    ...
    def __call__(self, eval_preds: "EvalPrediction", compute_result: bool = True) -> Optional[dict[str, float]]:
        ...
        bleu_score = self.corpus_bleu(decoded_preds, [[label] for label in decoded_labels])
        charf_score = self.charf.compute(predictions=decoded_preds, references=decoded_labels)
