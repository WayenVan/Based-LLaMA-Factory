    ...
    def __snyc_output_base_name(acc: Accelerator, base_name: str):
        ...
        with torch.no_grad():
            if acc.is_main_process:
                ...
                length = torch.tensor(len(bytes_base_name), dtype=torch.long, device=acc.device)
            else:
            ...
            if acc.is_main_process:
                content = torch.tensor(list(bytes_base_name), dtype=torch.uint8, device=acc.device)
...
class TrainingArguments(RayArguments, BaseTrainingArguments, AutoOutputTrainingArguments):
    ...
    def __post_init__(self):
        ...
        AutoOutputTrainingArguments.__post_init__(self)
